...
const store={
    get(k,def){ return def; }, // Luôn luôn trả về giá trị mặc định (false/Tắt)
    set(k,v){ /* Đã vô hiệu hóa tính năng lưu */ }
  };
...
(function(){
  ...
  const btnClearRam = document.querySelector('button[onclick*="runSysAction(\'RAM\')"]');
  if(btnClearRam) {
      btnClearRam.addEventListener("click", () => {
          setTimeout(() => {
              cleanRamStart = performance.now();
              ramSeries.fill(0.01);  
              pingSeries.fill(0.05); 
              
              if(typeof particles !== 'undefined' && particles.length > 0) {
                  particles.length = Math.floor(particles.length / 2);
              }
              log("[SYSTEM] Đã giải phóng RAM & Hạ Ping mạng thành công.", "i");
          }, 1200); 
      });
  }

  const btnOptCpu = document.querySelector('button[onclick*="runSysAction(\'CPU\')"]');
  if(btnOptCpu) {
      btnOptCpu.addEventListener("click", () => {
          setTimeout(() => {
              ...
              cpuSeries.fill(0.02); 
              gpuSeries.fill(0.02); 
              log("[SYSTEM] Đã tối ưu hóa luồng và hạ nhiệt CPU/GPU.", "i");
          }, 1200);
      });
  }


  const btnLockFps = document.getElementById('btn-lock-fps');
  if(btnLockFps) {
      btnLockFps.removeAttribute("onclick"); 
      
      btnLockFps.addEventListener("click", () => {
          lockedFps = !lockedFps; 
          const modeLabel = document.getElementById('cm-mode-now');
          
          if(lockedFps) {
              btnLockFps.innerText = "LOCK FPS: ON"; 
              btnLockFps.classList.remove('btn-danger'); 
              btnLockFps.style.color = "var(--primary)"; 
              btnLockFps.style.borderColor = "var(--primary)";
              if(modeLabel) modeLabel.textContent = `Locked 60`;
              log(`[SYSTEM] Đã ép khung hình hệ thống ở mức 60 FPS.`, "i");
          } else {
              btnLockFps.innerText = "LOCK FPS: OFF"; 
              btnLockFps.classList.add('btn-danger'); 
              btnLockFps.style.color = "var(--danger)"; 
              btnLockFps.style.borderColor = "var(--danger)";
              if(modeLabel) modeLabel.textContent = "Unlocked";
              log("[SYSTEM] Đã mở khóa giới hạn FPS.", "i");
          }
      });
  }
